...
const CryptoLayout = ({ children }) => {
  ...
  const navigationItems = [
    ...
    { name: "Notifications", path: "/cryptohub/notifications", icon: IoMdNotifications },
  ];
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 to-slate-800 text-white relative pt-24">
      {/* Sidebar */}
      <aside
        className={`fixed left-10 top-30 w-72 h-[calc(100vh-10rem)] z-30 bg-slate-900/95 border border-slate-700/50 shadow-xl text-white flex flex-col justify-between rounded-2xl backdrop-blur-sm overflow-hidden transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
      >
        <nav className="flex-1 w-full flex flex-col pt-8">
          <div className="px-6 mb-8">
            <h2 className="text-2xl font-bold tracking-wide text-cyan-300 mb-2">CRYPTO HUB</h2>
                ...
                <li key={item.path}>
                  <Link href={item.path}>
                    <Button
                      ...
                      className={`w-full flex items-center justify-start gap-3 text-base font-medium px-4 py-3 rounded-xl transition-all duration-200 relative group ${isActive ? "bg-slate-800/80 text-cyan-300 border border-cyan-500/30" : "bg-transparent text-slate-300 hover:bg-slate-800/50 hover:text-white border border-transparent hover:border-slate-700/50"}`}
                    >
                      <IconComponent className={`w-5 h-5 ${isActive ? "text-cyan-400" : "text-slate-400 group-hover:text-cyan-400"}`} />
                      <span>{item.name}</span>
                      {isActive && <div className="absolute right-3 w-2 h-2 bg-cyan-400 rounded-full"></div>}
                    </Button>
                  </Link>
                </li>
              );
              ...
              {userFromDbLoading ? (
                ...
                <>
                  ...
                  <div className="flex justify-around items-center text-center mb-4 text-sm">
                    ...
                    <div className="w-px h-8 bg-slate-700"></div> {/* Divider */}
                    <div>
