...
const PepeBurnsPage: React.FC = () => {
    ...
    const fetchPepeBurnData = async () => {
        ...
        try {
            ...
            console.log("🔎 PEPE Sheet Headers:", headers);

            const rows = data.values.slice(1).filter((row: string[]) => row.length > 1);

            // Column positions for PEPE data
            const finalDateIdx = 0; // Column A
            const finalCurrencyIdx = 1; // Column B
            const finalTransactionIdx = 2; // Column C
            const finalBurnCountIdx = 8; // Column I (PEPE burns)
            const burnAddressIdx = 8; // Column I (PEPE burn address - same column as burn count)

            console.log("🔎 PEPE Burn Address Column Index:", burnAddressIdx);
