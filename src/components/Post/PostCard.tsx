...
import { useDeletePostMutation, useGetAllPostsQuery, useIncreaseVideoViewCountMutation, useToggleLikeMutation, useUpdatePostMutation, useCreateCommentMutation } from "@/redux/features/api/postApi";
import { useAppSelector } from "@/redux/hooks";
import { Heart, MessageCircle, Edit, Trash, MoreHorizontal, Play, Eye, Send, User, ChevronDown, ChevronUp } from "lucide-react";
...
const PostCard = ({ post }: { post: Post }) => {
    ...
    const postCreatedDate = post?.createdAt ? format(new Date(post?.createdAt), 'MMM dd, yyyy') : '';
    ...
    return (
        <div
            ...
            onClick={(e) => {
                ...
                const isInteractive = target.closest('button') || target.closest('input') || target.closest('video') || target.closest('a');
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400">
                            @{currentPost?.author?.firstName?.toLowerCase()}{currentPost?.author?.lastName?.toLowerCase()}
                        </p>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-700/50">
                <div className="flex items-center gap-6">
                    ...
                    >
                        ...
                        />
                        <span className="text-sm font-medium">{currentPost?.likeCount}</span>
                    </button>
                    ...
                    >
                        ...
                        <span className="text-sm font-medium">{currentPost?.comments?.length || 0}</span>
                        {showComments ? (
                    ...
                    {(currentPost?.video || currentPost?.image) && (
                        <div className="flex items-center gap-2 text-slate-400">
                            ...
                            <span className="text-sm font-medium">{currentPost?.viewCount || 0}</span>
                        </div>
                    )}
                </div>
                    ...
                    <div className="space-y-3 max-h-60 overflow-y-auto">
                        {currentPost?.comments?.length > 0 ? (
                            currentPost.comments
                                ...
                                .map((comment, index) => (
                                    <div key={comment.id} className="flex items-start gap-3 p-3 bg-slate-700/30 rounded-lg">
