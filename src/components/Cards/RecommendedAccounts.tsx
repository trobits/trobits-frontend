...
import { useGetAllRecommendedUsersQuery, useToggleFollowMutation } from "@/redux/features/api/authApi";
...
const RecommendedAccounts = () => {
    ...
    useEffect(() => {
        if (allUsersData?.data) {
            ...
            setFilteredUsers(allUsersData.data); // Initialize filtered users
        }
    }, [ allUsersData ]);
    ...
    const handleFollow = async (followedId: string) => {
        if (!currentUser) {
            // Show toast message and do not proceed with the follow action
        ...
        <div>
            <Card className="border bg-transparent border-cyan-400 text-white">
                <CardHeader>
                    ...
                    <div className="flex items-center bg-black text-white border-2 border-cyan-500 px-2 py-1 rounded-md">
